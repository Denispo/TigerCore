...
class DataMapper
{
   ...
   private function validateProperty(mixed $valueToValidate, \ReflectionProperty $property):BaseParamErrorCode|null
   {
      ...
      foreach ($attributes as $oneAttribute) {
         ...
         if (
            (($valueToValidate instanceof ICanGetValueAsInit | is_int($valueToValidate)) && $attrInstance instanceof ICanAssertIntValue) ||
            (($valueToValidate instanceof ICanGetValueAsString | is_string($valueToValidate)) && $attrInstance instanceof ICanAssertStringValue) ||
            (($valueToValidate instanceof ICanGetValueAsFloat | is_float($valueToValidate)) && $attrInstance instanceof ICanAssertFloatValue) ||
            (($valueToValidate instanceof ICanGetValueAsBoolean | is_bool($valueToValidate)) && $attrInstance instanceof ICanAssertBooleanValue)
         ){
            ...
            //TODO: should notify someone that we want can not runAssertion for this $valueToValidate
         }
      }
      return null;
   }
